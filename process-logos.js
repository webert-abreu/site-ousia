...
async function processLogos() {
  ...
  for (let y = 0; y < h1; y++) {
    for (let x = 0; x < w1; x++) {
      ...
      // Se pixel não for o fundo escuro (R > 55 ou G > 50 ou B > 50)
      if (r > 55 || g > 50 || b > 50) {
        if (x < minX1) minX1 = x;
        if (x > maxX1) maxX1 = x;
        if (y < minY1) minY1 = y;
  ...
  // Criar versão transparente da logo escura (remover fundo escuro)
  imgEscuro.scan(0, 0, imgEscuro.width, imgEscuro.height, function (x, y, idx) {
    const r = this.bitmap.data[idx + 0];
    const g = this.bitmap.data[idx + 1];
    const b = this.bitmap.data[idx + 2];

    // Se for pixel escuro do fundo, torna transparente
    if (r < 60 && g < 55 && b < 55) {
      this.bitmap.data[idx + 3] = 0;
    } else {
      // Suaviza a transição de opacidade
      const brightness = (r + g + b) / 3;
      const alpha = Math.min(255, Math.max(0, (brightness - 40) * 1.5));
      this.bitmap.data[idx + 3] = alpha;
    }
  });

  await imgEscuro.write(path.join(logoDir, 'logo-ousia-escuro-transparente.png'));
  console.log('Logo escuro transparente salvo com sucesso!');
  ...
  for (let y = 0; y < h2; y++) {
    for (let x = 0; x < w2; x++) {
      ...
      // Se pixel não for fundo branco (R < 235 ou G < 235 ou B < 235)
  ...
  // Criar versão transparente da logo clara (remover fundo branco)
  imgClaro.scan(0, 0, imgClaro.width, imgClaro.height, function (x, y, idx) {
    const r = this.bitmap.data[idx + 0];
    const g = this.bitmap.data[idx + 1];
    const b = this.bitmap.data[idx + 2];

    // Se for pixel claro do fundo, torna transparente
    if (r > 220 && g > 220 && b > 220) {
      this.bitmap.data[idx + 3] = 0;
    } else {
      const darkness = 255 - ((r + g + b) / 3);
      const alpha = Math.min(255, Math.max(0, darkness * 1.6));
      this.bitmap.data[idx + 3] = alpha;
      // Garante que o texto seja a cor de chocolate escuro #1C1514
      this.bitmap.data[idx + 0] = 28;
      this.bitmap.data[idx + 1] = 21;
      this.bitmap.data[idx + 2] = 20;
    }
  });

  await imgClaro.write(path.join(logoDir, 'logo-ousia-claro-transparente.png'));
  console.log('Logo claro transparente salvo com sucesso!');
}
